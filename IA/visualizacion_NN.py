...
class visualizador_resultados():
    ...
    def plot(self):
        ...
        self.ax0.set_position([.25/4, 0.3, 0.25, 0.5])
        ...
        self.im0 = self.ax0.pcolormesh(self.frecuencias, self.delays, self.T[0][:].reshape(2*N - 1, N), cmap='inferno')
        ...
        self.im1 = self.ax1.pcolormesh(self.frecuencias, self.delays, self.T_pred[0].reshape(2*N - 1, N), cmap='inferno')
    ...
    def next_result(self, event):
        ...
        match self.mode:
            case 'random':
                ...
                self.fig.suptitle(f"TBP = {self.TBP[self.random_index_order[self.last_index]]:.2f}      Error en la traza = {self.errores_traza[self.random_index_order[self.last_index]]:.2E}", fontweight='bold')

                self.im0.set_array(self.T[self.random_index_order[self.last_index]].reshape(2*N - 1, N))
            ...
            case 'best':
                ...
                self.fig.suptitle(f"TBP = {self.TBP[self.best_index_order[self.last_index]]:.2f}      Error en la traza = {self.errores_traza[self.best_index_order[self.last_index]]:.2E}", fontweight='bold')

                self.im0.set_array(self.T[self.best_index_order[self.last_index]].reshape(2*N - 1, N))
                self.im0.set_clim(np.min(self.T[self.best_index_order[self.last_index]]), np.max(self.T[self.best_index_order[self.last_index]]))
                
                self.im1.set_array(self.T_pred[self.best_index_order[self.last_index]].reshape(2*N - 1, N))
                ...
                diff = np.abs(self.T[self.best_index_order[self.last_index]] - self.T_pred[self.best_index_order[self.last_index]]).reshape(2*N - 1, N)
                self.im2.set_array(diff)
                self.im2.set_clim(np.min(diff), np.max(diff))

            
            case 'worst':
                self.fig.suptitle(f"TBP = {self.TBP[self.worst_index_order[self.last_index]]:.2f}      Error en la traza = {self.errores_traza[self.worst_index_order[self.last_index]]:.2E}", fontweight='bold')

                self.im0.set_array(self.T[self.worst_index_order[self.last_index]].reshape(2*N - 1, N))
                self.im0.set_clim(np.min(self.T[self.worst_index_order[self.last_index]]), np.max(self.T[self.worst_index_order[self.last_index]]))

                self.im1.set_array(self.T_pred[self.worst_index_order[self.last_index]].reshape(2*N - 1, N))
                self.im1.set_clim(np.min(self.T_pred[self.worst_index_order[self.last_index]]), np.max(self.T_pred[self.worst_index_order[self.last_index]]))

                diff = np.abs(self.T[self.worst_index_order[self.last_index]] - self.T_pred[self.worst_index_order[self.last_index]]).reshape(2*N - 1, N)
                self.im2.set_array(diff)
                self.im2.set_array(diff)
                self.im2.set_clim(np.min(diff), np.max(diff))
