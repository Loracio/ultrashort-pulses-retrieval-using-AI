...
def pulso_gaussiano(t, t0, A, τ, ω_0, φ):
    ...
    return A * np.exp(-(t-t0)*(t-t0) / (2 * τ)) * np.exp(1j * ( ω_0 * t + φ ))
...
def transformada_pulso_gaussiano(ω, t0, A, τ, ω_0, φ):
    ...
    El pulso viene dado por:
        f(t) = A * exp(-(t - t₀)² / 2*τ) * exp(i * ( ω_0 * t + φ) )

    Su transformada de Fourier será:
        F(ω) = A  * sqrt(2π * τ) * exp(i * (t₀ * (ω₀ - ω) +  φ)) * exp(-(ω - ω₀)²  / (2 * τ))
    ...
    return A * np.sqrt(2 * np.pi * τ) * np.exp(1j * (φ + t0 * (ω_0 - ω)) - (ω - ω_0)*(ω - ω_0) / (2 * τ))